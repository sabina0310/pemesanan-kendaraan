<?php 
  ...
  if((isset($_GET['aksi']))&&(isset($_GET['data']))){
    if($_GET['aksi']=='setuju'){
      ...
      //setuju 
      $sql_dh = "UPDATE pesan set `status`= 'Selesai'
      where `id_pesan` = '$id_pesan'";
      mysqli_query($koneksi,$sql_dh);
...
<!-- Content Header (Page header) -->
  <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3><i class="fas fa-newspaper"></i> Dipesan</h3>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item active"> Dipesan</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                    ...
                    <tr>
                      ...
                      <td align="center">
                        <a href="javascript:if(confirm('Anda yakin ingin batalkan pesanan <?php echo  $id_pesan; ?>?')) 
                          window.location.href ='index.php?include=dipesan&aksi=batal&data=<?php echo  $id_pesan; ?>&notif=batalkanberhasil'"
                          class="btn btn-xs btn-danger" title="Batalkan"><i class="fas fa-times"></i></a>
                        <a href="javascript:if(confirm('Anda yakin ingin menyelesaikan pesanan <?php echo $id_pesan; ?>?')) 
                          window.location.href ='index.php?include=dipesan&aksi=setuju&data=<?php echo  $id_pesan; ?>&notif=setujuiberhasil'"
                          class="btn btn-xs btn-success" title="Selesai"><i class="fas fa-check"></i></a>
                      </td>
                    </tr>
                    <?php $posisi++;}?>
                  </tbody>
              ...
              if($jum_halaman==0){
                //tidak ada halaman
                  ...
                  if($halaman!=1){
                    ...
                    href='index.php?include=dipesan&halaman=1'>First</a></li>";
                    echo "<li class='page-item'><a class='page-link' 
                    href='index.php?include=dipesan&halaman=$sebelum'>«</a></li>";
                  }
                  for($i=1; $i<=$jum_halaman; $i++){
                    if ($i > $halaman - 5 and $i < $halaman + 5 ) {
                      if($i!=$halaman){
                          echo "<li class='page-item'><a class='page-link' 
                          href='index.php?include=dipesan&halaman=$i'>$i</a></li>";
                      }else{
                  ...
                  if($halaman!=$jum_halaman){
                    echo "<li class='page-item'><a class='page-link' 
                    href='index.php?include=dipesan&halaman=$setelah'>»</a></li>";
                    echo "<li class='page-item'><a class='page-link' 
                    href='index.php?include=dipesan&halaman=$jum_halaman'>Last</a></li>";
                  }
