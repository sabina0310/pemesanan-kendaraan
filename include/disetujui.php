<?php 
  ...
  if((isset($_GET['aksi']))&&(isset($_GET['data']))){
    if($_GET['aksi']=='setuju'){
      $id_pesan= $_GET['data'];
      //setuju 
      $sql_dh = "UPDATE pesan set `status`= 'Dipesan'
      where `id_pesan` = '$id_pesan'";
      mysqli_query($koneksi,$sql_dh);
    }
  }
...
<!-- Content Header (Page header) -->
  <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3><i class="fas fa-newspaper"></i> Disetujui</h3>
          </div>
          <div class="col-sm-6">
                    ...
                    <tr>
                      ...
                      <td align="center">
                        <a href="javascript:if(confirm('Anda yakin ingin batalkan pesanan <?php echo  $id_pesan; ?>?')) 
                          window.location.href ='index.php?include=disetujui&aksi=batal&data=<?php echo  $id_pesan; ?>&notif=batalkanberhasil'"
                        ...
                        <a href="javascript:if(confirm('Anda yakin ingin setujui pesanan <?php echo $id_pesan; ?>?')) 
                          window.location.href ='index.php?include=disetujui&aksi=setuju&data=<?php echo  $id_pesan; ?>&notif=setujuiberhasil'"
              ...
              if($jum_halaman==0){
                //tidak ada halaman
                  ...
                  if($halaman!=1){
                    ...
                    href='index.php?include=disetujui&halaman=1'>First</a></li>";
                    echo "<li class='page-item'><a class='page-link' 
                    href='index.php?include=disetujui&halaman=$sebelum'>«</a></li>";
                  }
                  for($i=1; $i<=$jum_halaman; $i++){
                    if ($i > $halaman - 5 and $i < $halaman + 5 ) {
                      if($i!=$halaman){
                          echo "<li class='page-item'><a class='page-link' 
                          href='index.php?include=disetujui&halaman=$i'>$i</a></li>";
                      }else{
                  ...
                  if($halaman!=$jum_halaman){
                    echo "<li class='page-item'><a class='page-link' 
                    href='index.php?include=disetujui&halaman=$setelah'>»</a></li>";
                    echo "<li class='page-item'><a class='page-link' 
                    href='index.php?include=disetujui&halaman=$jum_halaman'>Last</a></li>";
                  }
